...
For imports that create catalog scaffolding and then apply dependent session records, add an orchestration-level regression in addition to parser and service unit tests. It must run the production order and prove that categories, participants, and parent entrants are linked to the selected event and session before dependent records are validated or applied. Include a second event in the fixture to prove that no scaffold entity leaks to an unrelated or fixture event. When an error message names an imported participant, test its user-facing display label, including the unknown-transmitter placeholder label for blank names. Unit tests of parser output or catalog persistence alone do not satisfy this requirement.
