...
import { readParticipantsXlsxFromBuffer } from "../controllers/participant.ts";
...
export class OutreachTeamsRaceTestSession
  ...
  private _resourceProvider: ResourceProvider<string>;
  constructor(resourceProvider: ResourceProvider<string>, state?: RaceState) {
    super(state);
    this._resourceProvider = resourceProvider;
  }
  
  public static async create(rp: ResourceProvider<string>): Promise<OutreachTeamsRaceTestSession> {
    return Promise.resolve(new OutreachTeamsRaceTestSession(rp, {} as RaceState));
  };
  ...
  // return readParticipantsXlsx(
    this._resourceProvider.getResourceBuf(ENTRIES_DATA_FILE)
      .then((buffer) => 
        readParticipantsXlsxFromBuffer(buffer,
