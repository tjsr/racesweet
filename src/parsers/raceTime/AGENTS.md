...
- When SRT raw records can be matched to DBF, NO1/NO2, AT1, or AT2 rows for the same transmitter or plate and time, keep a single imported crossing. Prefer the structured DBF-style row as the imported source record, but fill missing line, loop, confidence-factor, and hit-count metadata from the SRT row.
- To derive the SRT-to-DBF time offset for a session, first filter SRT records to transponders allocated to entrants in that session, then reduce to line 1 crossings to choose the transmitter with the median line 1 crossing count. After that, only compare that selected transmitter's DBF rows with that selected transmitter's SRT rows. Score candidate offsets from this small pair of collections by the longest consecutive matching run, then by total matching crossings and smallest timing delta. Apply the winning offset to all MR-SCATS rows for the session so as many DBF/NO/AT crossings as possible match SRT time-of-day values.
- Multi-session SRT files may contain crossings from an earlier session before the target session's start marker. Choose the raw SRT segment whose derived `40` start-of-race time is closest to the session's programme start, and do not import pre-segment rows into the target session.
- If SRT and DBF/NO/AT rows disagree about an event start because DBF elapsed values only line up after anchoring to matched SRT visible times, derive the green flag from the anchored elapsed-zero plus any explicit green/start elapsed offset rather than from an unrelated earlier programme clock.
