...
describe('parseDateTime', () => {
  ...
  it('Should parse a time only', () => {
    ...
    const chipLineTime = chipLine.split(',')[1];
    const parsed: Date = parseDateTime(chipLineTime, dateHint, msTimeFormat);
  ...
  it('Should parse a time with a date hint', () => {
    const dateHint = new TZDate('2023-08-25', getUserTimezone());
    const chipLine = '1245,19:11:06.405';
    const chipLineTime = chipLine.split(',')[1];
    const parsed: Date = parseDateTime(chipLineTime, dateHint, msTimeFormat);
    expect(parsed).toBeDefined();
