...
import type {
  EventParticipant,
  EventParticipantId,
  ParticipantTransponder,
  ParticipateRacePlate,
} from "../model/eventparticipant.js";
import {
  createCategoryId,
...
const createPersistence = (
  initial = createDefaultEventCatalogLedger(),
): EventCatalogPersistence => {
  let ledger = initial;
...
const SEED_CLUBMAN_CATEGORY_ID = createCategoryId(
  "event-2026-racesweet-round-1-category-clubman",
);
const SEED_ENTRANT_ID = createEventEntrantId(
  "event-2026-racesweet-round-1-entrant-101",
);
...
const SEED_PREMIER_CATEGORY_ID = createCategoryId(
  "event-2026-racesweet-round-1-category-premier",
);
...
const getParticipantRacePlate = (
  ...
  const racePlateIdentifier = participant.identifiers.find((identifier) => {
    return (
      "racePlate" in identifier && typeof identifier.racePlate === "string"
    );
...
const expectLedgerPhaseIdsToBeValid = (
  ledger: EventCatalogLedger | undefined,
): EventCatalogState => {
  expect(ledger).toBeDefined();
  expect(ledger?.mutations.every((mutation) => validateUuid(mutation.id))).toBe(
    true,
  );
...
describe("EventCatalogService", () => {
  it("rejects malformed persistence objects before loading the event catalog", async () => {
    await expect(
      EventCatalogService.create({
        load: vi.fn(),
      } as unknown as EventCatalogPersistence),
    ).rejects.toThrow(
  ...
  it("creates seed fixture data with UUID-compatible, internally linked IDs", () => {
    ...
    const seedEntrant = state.entrants.find(
      (entrant) => entrant.id === SEED_ENTRANT_ID,
    );

    expectCatalogStateIdsToBeValid(state);
    expect(seedEvent).toBeDefined();
    expect(seedEvent?.categoryIds).toEqual([
      SEED_PREMIER_CATEGORY_ID,
      SEED_CLUBMAN_CATEGORY_ID,
    ]);
    expect(seedEvent?.entrantIds).toEqual([SEED_ENTRANT_ID]);
    expect(seedEvent?.sessionIds).toEqual([
    ...
    expect(state.categories.map((category) => category.eventId)).toEqual([
      SEED_EVENT_ID,
      SEED_EVENT_ID,
    ]);
    expect(state.sessions.map((session) => session.eventId)).toEqual([
      SEED_EVENT_ID,
      SEED_EVENT_ID,
      SEED_EVENT_ID,
    ]);
    expect(seedEntrant?.categoryIds).toEqual([SEED_PREMIER_CATEGORY_ID]);
    expect(seedEntrant?.sessionIds).toEqual([
      SEED_PRACTICE_SESSION_ID,
      SEED_QUALIFYING_SESSION_ID,
      SEED_RACE_SESSION_ID,
    ]);
  ...
  it("finds entrant memberships for a participant across any event to diagnose broken links", async () => {
    const currentEventId = createEventId(
      "participant-membership-current-event",
    );
    const previousEventId = createEventId(
      "participant-membership-previous-event",
    );
    const previousCategoryId = createCategoryId(
      "participant-membership-previous-category",
    );
    const previousSessionId = createSessionId(
      "participant-membership-previous-session",
    );
    const participantId = createEventParticipantId(
      "participant-membership-rider",
    );
    ...
    const memberships =
      service.findEntrantMembershipsForParticipant(participantId);

    expect(
      service.findEntrantMembershipsForParticipant(
        participantId,
        currentEventId,
      ),
    ).toEqual([]);
    expect(
      getParticipantEntrantMemberships(service.catalog, participantId, {
        eventId: previousEventId,
      }).map((membership) => membership.entrant.id),
    ).toEqual([participantId]);
    expect(memberships.map((membership) => membership.entrant.id)).toEqual([
      participantId,
      teamEntrantId,
    ]);
    expect(memberships.map((membership) => membership.event.id)).toEqual([
      previousEventId,
      previousEventId,
    ]);
    expect(
      memberships.map((membership) =>
        membership.categories.map((category) => category.id),
      ),
    ).toEqual([[previousCategoryId], [previousCategoryId]]);
    expect(
      memberships.map((membership) =>
        membership.sessions.map((session) => session.id),
      ),
    ).toEqual([[previousSessionId], [previousSessionId]]);
  });
  ...
  it("links a globally known entrant into the imported event when scaffold data references it", async () => {
    ...
    const currentCategoryId = createCategoryId(
      "global-entrant-current-category",
    );
    const previousEventId = createEventId("global-entrant-previous-event");
    const previousCategoryId = createCategoryId(
      "global-entrant-previous-category",
    );
    const entrantId = createEventEntrantId("global-entrant-existing");
    const participantId = createEventParticipantId(
      "global-entrant-participant",
    );
    const ledger: EventCatalogLedger = {
      mutations: [
        {
    ...
    const currentEntrants = getEntrantsForEvent(
      service.catalog,
      currentEventId,
    );
    const previousEvent = service.catalog.events.find(
      (event) => event.id === previousEventId,
    );
    const currentEvent = service.catalog.events.find(
      (event) => event.id === currentEventId,
    );
    const movedEntrant = currentEntrants.find(
      (entrant) => entrant.id === entrantId,
    );
    const savedLedger = vi
      .mocked(persistence.save)
      .mock.calls.at(-1)?.[0] as EventCatalogLedger;

    expect(previousEvent?.entrantIds).not.toContain(entrantId);
    expect(currentEvent?.entrantIds).toContain(entrantId);
    expect(movedEntrant).toEqual(
    ...
    expect(
      getEntrantsForEvent(service.catalog, previousEventId).find(
        (entrant) => entrant.id === entrantId,
      ),
    ).toBeUndefined();
    expect(savedLedger.mutations).toEqual(
  ...
  it("persists imported participant identifier updates to the ledger and reloads them", async () => {
    ...
    await service.updateImportedRaceState(
      SEED_EVENT_ID,
      SEED_PRACTICE_SESSION_ID,
      raceState,
    );
    ...
    const savedMutation = savedLedger.mutations.findLast(
      (mutation) => mutation.type === "race-state-imported",
    );
    expect(savedMutation?.type).toBe("race-state-imported");
    expect(
      savedMutation && savedMutation.type === "race-state-imported"
        ? savedMutation.raceState.participants?.[0].identifiers
        : [],
    ).toEqual(raceState.participants[0].identifiers);

    const reloaded = await EventCatalogService.create(
      createPersistence(savedLedger),
    );
    const reloadedRaceState = reloaded.getImportedRaceState(
      SEED_EVENT_ID,
      SEED_PRACTICE_SESSION_ID,
    );
    expect(reloadedRaceState?.participants?.[0].identifiers).toEqual(
      raceState.participants[0].identifiers,
    );
  });
  ...
  it("repairs imported participants without entrant IDs into direct parent entrants", async () => {
    ...
    const service = await EventCatalogService.create(persistence);
    const participantId = createEventParticipantId(
      "blank-imported-entrant-participant",
    );
    ...
    const importedRaceState = service.getImportedRaceState(
      SEED_EVENT_ID,
      SEED_PRACTICE_SESSION_ID,
    );
    const entrant = getEntrantsForEvent(service.catalog, SEED_EVENT_ID).find(
      (item) => item.id === participantId,
    );

    expect(importedRaceState?.participants?.[0]).toEqual(
      expect.objectContaining({
  ...
  it("imports motorsport entrants by transmitter, enriches placeholders, and keeps multiple team vehicles distinct", async () => {
    ...
    const placeholderEntrantId = createEventEntrantId(
      "entrant-import-placeholder",
    );
    const placeholderParticipantId = createEventParticipantId(
      "entrant-import-placeholder",
    );
    const placeholderParticipant: EventParticipant = {
      categoryId: SEED_PREMIER_CATEGORY_ID,
      currentResult: undefined,
      entrantId: placeholderEntrantId,
      firstname: "",
      id: placeholderParticipantId,
      identifiers: [
        { fromTime: undefined, toTime: undefined, txNo: "98" },
      ] as ParticipantTransponder[],
      lastRecordTime: null,
      resultDuration: null,
      surname: "",
    };
    ...
    const importedRiders = getEntrantsForEvent(catalog, SEED_EVENT_ID).filter(
      (entrant) =>
        entrant.name === "Rick Mears" || entrant.name === "Second Driver",
    );
    ...
    const enrichedParticipant = importedRaceState?.participants?.find(
      (participant) => participant.id === placeholderParticipantId,
    );
  ...
  it("matches entrant imports by driver name and then reuses the allocated race number", async () => {
    ...
    await service.createEntrant(SEED_EVENT_ID, "rider");
    const createdEntrant = getEntrantsForEvent(
      service.catalog,
      SEED_EVENT_ID,
    ).find((entrant) => entrant.name === "New Driver");
    expect(createdEntrant).toBeDefined();
    await service.updateEntrant(createdEntrant!.id, {
      firstName: "Casey",
      lastName: "Jones",
      name: "Casey Jones",
    });

    await service.importEntrants(SEED_EVENT_ID, [
      {
        firstName: "Casey",
    ...
    await service.importEntrants(SEED_EVENT_ID, [
      {
        firstName: "Casey",
        fullName: "Casey Jones-Smith",
        lastName: "Jones-Smith",
        raceNumber: "44",
        transponderNumber: "744",
      },
    ]);
    ...
    expect(updatedEntrant).toEqual(
      expect.objectContaining({
        firstName: "Casey",
        lastName: "Jones-Smith",
        name: "Casey Jones-Smith",
      }),
    );
    expect(updatedEntrant?.identifiers).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ racePlate: "44" }),
        expect.objectContaining({ txNo: "744" }),
      ]),
    );
    expect(
  ...
  it("rejects an entrant import when a transmitter is already assigned to multiple entrants", async () => {
    ...
    await service.createEntrant(SEED_EVENT_ID, "rider");
    const duplicateEntrant = getEntrantsForEvent(
      service.catalog,
      SEED_EVENT_ID,
    ).find((entrant) => entrant.name === "New Driver");
    const sharedTransponder = [
      { fromTime: undefined, toTime: undefined, txNo: "9001" },
    ] as ParticipantTransponder[];
    await service.updateEntrant(SEED_ENTRANT_ID, {
      identifiers: sharedTransponder,
    });
    await service.updateEntrant(duplicateEntrant!.id, {
      name: "Second Driver",
      identifiers: sharedTransponder,
    });

    await expect(
      service.importEntrants(SEED_EVENT_ID, [{ transponderNumber: "9001" }]),
    ).rejects.toThrow(
      `non-placeholder competitor(s) Rider 101 [${SEED_ENTRANT_ID}], Second Driver [${duplicateEntrant!.id}]`,
    );
  });

  it("updates a full grid of existing transmitter placeholders without replacing participant IDs", async () => {
    const service = await EventCatalogService.create(
      createPersistence(createSeedEventCatalogLedger()),
    );
    const placeholderParticipants = Array.from(
      ...
      (_, index): EventParticipant => {
        const transmitterNumber = index + 1;
        return {
          categoryId: SEED_PREMIER_CATEGORY_ID,
          currentResult: undefined,
          entrantId: createEventEntrantId(
            `full-grid-placeholder-${transmitterNumber}`,
          ),
          firstname: "",
          id: createEventParticipantId(
            `full-grid-placeholder-${transmitterNumber}`,
          ),
          identifiers: [
            { fromTime: undefined, toTime: undefined, txNo: transmitterNumber },
          ] as ParticipantTransponder[],
          lastRecordTime: null,
          resultDuration: null,
          surname: "",
    ...
    const importRecords = placeholderParticipants
      .slice(0, 33)
      .map((participant, index) => ({
        entrantName: `Team ${Math.floor(index / 3) + 1}`,
        firstName: `Driver${index + 1}`,
        fullName: `Driver${index + 1} Surname${index + 1}`,
        lastName: `Surname${index + 1}`,
        raceNumber: `${index + 101}`,
        startOrder: index + 1,
        transponderNumber: `${index + 1}`,
        vehicle: `Vehicle ${index + 1}`,
      }));

    await service.importEntrants(SEED_EVENT_ID, importRecords);

    const importedRaceState = service.getImportedRaceState(
      SEED_EVENT_ID,
      SEED_RACE_SESSION_ID,
    );
    expect(importedRaceState?.participants).toHaveLength(34);
    placeholderParticipants.slice(0, 33).forEach((placeholder, index) => {
      const participant = importedRaceState?.participants?.find(
        (candidate) => candidate.id === placeholder.id,
      );
      const entrant = getEntrantsForEvent(service.catalog, SEED_EVENT_ID).find(
        (candidate) => candidate.id === placeholder.entrantId,
      );
      ...
      expect(entrant).toEqual(
        expect.objectContaining({
          ...
          name: `Driver${index + 1} Surname${index + 1}`,
          categoryId: SEED_PREMIER_CATEGORY_ID,
          categoryIds: [SEED_PREMIER_CATEGORY_ID],
    ...
    const unmatchedParticipant = importedRaceState?.participants?.find(
      (candidate) => candidate.id === placeholderParticipants[33]?.id,
    );
    expect(unmatchedParticipant?.firstname).toBe("");
    expect(unmatchedParticipant?.surname).toBe("");
  });
  ...
  it("creates a parent entrant for an imported unknown-transmitter participant", async () => {
    ...
    const participantId = createEventParticipantId(
      "unknown-transmitter-participant",
    );
    ...
    expect(
      service.catalog.events.find((event) => event.id === SEED_EVENT_ID)
        ?.entrantIds,
    ).toContain(entrantId);
  });
  ...
  it("adds unknown-transmitter placeholder entrants only to the imported event", async () => {
    ...
    const targetEvent = service.catalog.events.find(
      (event) => event.id !== SEED_EVENT_ID,
    );
    expect(targetEvent).toBeDefined();
    await service.createSession(targetEvent!.id);
    const targetSession = getSessionsForEvent(
      service.catalog,
      targetEvent!.id,
    )[0];
    const categoryId = createCategoryId(
      "target-event-unknown-transmitter-category",
    );
    const entrantId = createEventEntrantId(
      "target-event-unknown-transmitter-entrant",
    );
    const participantId = createEventParticipantId(
      "target-event-unknown-transmitter-participant",
    );
    ...
    const placeholderEntrant = service.catalog.entrants.find(
      (entrant) => entrant.id === entrantId,
    );
    const importedEvent = service.catalog.events.find(
      (event) => event.id === targetEvent!.id,
    );
    const otherEvent = service.catalog.events.find(
      (event) => event.id === SEED_EVENT_ID,
    );
    ...
    expect(
      getEntrantsForEvent(service.catalog, targetEvent!.id).map(
        (entrant) => entrant.id,
      ),
    ).toContain(entrantId);
  });
  ...
  it("does not delete imported parent entrants while participants still reference them", async () => {
    ...
    const service = await EventCatalogService.create(persistence);
    const participantId = createEventParticipantId(
      "delete-guard-imported-participant",
    );
    ...
    await expect(
      service.deleteEntrant(SEED_EVENT_ID, entrantId),
    ).rejects.toThrow(
  ...
  it("returns team members with their team when selecting entrants for a team category", () => {
    ...
    expect(
      getEntrantsForCategory(state, eventId, categoryId).map(
        (entrant) => entrant.name,
      ),
    ).toEqual(["Fast Friends", "Alice RIDER"]);
  });

  it("validates seed data through load, save, callback, and service state phases", async () => {
    const emptyPersistence = createPersistence();
    const onPersistedLedger: (ledger: EventCatalogLedger) => Promise<void> =
      vi.fn(async (_ledger: EventCatalogLedger) => undefined);
  ...
  it("cleans duplicate mutation IDs while loading and saves the repaired ledger", async () => {
    const seededLedger = createSeedEventCatalogLedger();
    const duplicateMutationId = createId(
      "mutation-duplicate-loaded-event-name",
    );
    ...
    const savedLedger = vi
      .mocked(persistence.save)
      .mock.calls.at(-1)?.[0] as EventCatalogLedger;
    const loadedEvent = service.catalog.events.find(
      (event) => event.id === SEED_EVENT_ID,
    );
    expect(loadedEvent?.name).toBe("Final Source Name");
    expect(
      savedLedger.mutations.filter(
        (mutation) => mutation.id === duplicateMutationId,
      ),
    ).toHaveLength(1);
  ...
  it("repairs loaded ledgers by rewriting IDs and restoring parent child relationships before use", async () => {
    ...
    const service = await EventCatalogService.create(rawPersistence, {
      onPersistedLedger,
    });
  ...
  it("marks an event as deleted while leaving child catalog data untouched and hidden from event-scoped lookups", async () => {
    ...
    expect(
      service.catalog.events.find((event) => event.id === TEST_EVENT_ID),
    ).toBeUndefined();
    ...
    expect(
      service.catalog.sessions.find(
        (session) => session.id === TEST_SESSION_ID,
      ),
    ).toBeDefined();
    expect(
      service.catalog.categories.find(
        (category) => category.id === TEST_CATEGORY_ID,
      ),
    ).toBeDefined();
    expect(
      service.catalog.entrants.find(
        (entrant) => entrant.id === TEST_ENTRANT_ID,
      ),
    ).toBeDefined();
    expect(seededPersistence.save).toHaveBeenCalledOnce();
    expect(onPersistedLedger).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("activates a session and its event through the ledger, persistence, and upstream callback flow", async () => {
    ...
    const session = service.catalog.sessions.find(
      (item) => item.id === SEED_RACE_SESSION_ID,
    );
    expect(service.catalog.activeEventId).toBe(SEED_EVENT_ID);
  ...
  it("moves a session between parent events through one persisted ledger flow", async () => {
    ...
    const movedSession = service.catalog.sessions.find(
      (session) => session.id === SEED_QUALIFYING_SESSION_ID,
    );
    const sourceEvent = service.catalog.events.find(
      (event) => event.id === SEED_EVENT_ID,
    );
    const targetEvent = service.catalog.events.find(
      (event) => event.id === TEST_EVENT_ID,
    );
    ...
    expect(
      getSessionsForEvent(service.catalog, SEED_EVENT_ID).map(
        (session) => session.id,
      ),
    ).not.toContain(SEED_QUALIFYING_SESSION_ID);
    expect(
      getSessionsForEvent(service.catalog, TEST_EVENT_ID).map(
        (session) => session.id,
      ),
    ).toContain(SEED_QUALIFYING_SESSION_ID);
    expect(seededPersistence.save).toHaveBeenCalledOnce();
    expect(onPersistedLedger).toHaveBeenCalledWith(
      expect.objectContaining({
        mutations: expect.arrayContaining([
          ...
          expect.objectContaining({
            changes: {
              sessionIds: [SEED_PRACTICE_SESSION_ID, SEED_RACE_SESSION_ID],
            },
            eventId: SEED_EVENT_ID,
            type: "event-updated",
          }),
  ...
  it("creates an event through the ledger, persistence, and upstream callback flow", async () => {
    ...
    const createdEvent = service.catalog.events.find(
  ...
  it("creates team entrants through the ledger, persistence, and upstream callback flow", async () => {
    ...
    const createdTeam = service.catalog.entrants.find(
      (entrant) => entrant.name === "New Team",
    );
  ...
  it("updates event, session, and category rule details through immutable ledger changes", async () => {
    ...
    await service.updateEvent(SEED_EVENT_ID, {
      ...
      trackMap: {
        ...
        timingLines: [{ label: "Finish", lineNumber: 1, progress: 0.75 }],
        trackCsvContent:
    ...
    const event = service.catalog.events.find(
      (item) => item.id === SEED_EVENT_ID,
    );
    const session = service.catalog.sessions.find(
      (item) => item.id === SEED_PRACTICE_SESSION_ID,
    );
    const raceSession = service.catalog.sessions.find(
      (item) => item.id === SEED_RACE_SESSION_ID,
    );
    const category = service.catalog.categories.find(
      (item) => item.id === SEED_PREMIER_CATEGORY_ID,
    );
    ...
    expect(onPersistedLedger).toHaveBeenCalledTimes(4);
    expect(onPersistedLedger).toHaveBeenCalledWith(
      expect.objectContaining({
        mutations: expect.arrayContaining([
          expect.objectContaining({
            changes: expect.objectContaining({ trackMap: expect.any(Object) }),
            eventId: SEED_EVENT_ID,
            type: "event-updated",
          }),
        ]),
  ...
  it("replaces a session import without retaining its stale categories or entrants", async () => {
    ...
    const staleParticipantId = createEventParticipantId(
      "stale-ctc-import-participant",
    );
    ...
    await service.replaceImportedRaceState(
      SEED_EVENT_ID,
      SEED_PRACTICE_SESSION_ID,
      {
        categories: [],
    ...
    const after = service.catalog.sessions.find(
      (session) => session.id === SEED_PRACTICE_SESSION_ID,
    );
    const event = service.catalog.events.find(
      (item) => item.id === SEED_EVENT_ID,
    );
    ...
    expect(
      getCategoriesForEvent(service.catalog, SEED_EVENT_ID).find(
        (category) => category.id === staleCategoryId,
      ),
    ).toBeUndefined();
    expect(
      getEntrantsForEvent(service.catalog, SEED_EVENT_ID).find(
        (entrant) => entrant.id === staleEntrantId,
      ),
    ).toBeUndefined();
  ...
  it("repairs persisted cross-event category links and rejects future session assignments", async () => {
    ...
    const service = await EventCatalogService.create(persistence);
    await service.createEvent();
    const secondEventId = service.catalog.events.find(
      (event) => event.id !== SEED_EVENT_ID,
    )?.id;
    expect(secondEventId).toBeDefined();
    await service.createSession(secondEventId!);
    const secondSessionId = service.catalog.sessions.find(
      (session) => session.eventId === secondEventId,
    )?.id;
    ...
    const repairedEvent = repairedService.catalog.events.find(
      (event) => event.id === secondEventId,
    );
    const repairedSession = repairedService.catalog.sessions.find(
    ...
    await expect(
      repairedService.updateSession(secondSessionId!, {
        categoryIds: [SEED_PREMIER_CATEGORY_ID],
      }),
  ...
  it("applies a batch import incrementally when the ledger has a long mutation history", async () => {
    const seed = createSeedEventCatalogLedger();
    const historicalMutations: EventCatalogLedger["mutations"] = Array.from(
      { length: 10_000 },
      (_, index) => ({
        changes: {
          name: index % 2 === 0 ? "Historical Event A" : "Historical Event B",
        },
        eventId: SEED_EVENT_ID,
        id: createId(`long-ledger-event-update-${index}`),
        timestamp: `2026-07-17T00:${(index % 60).toString().padStart(2, "0")}:00.000Z`,
        type: "event-updated" as const,
      }),
    );
    const persistence = createPersistence({
      ...seed,
      mutations: [...seed.mutations, ...historicalMutations],
    });
    const service = await EventCatalogService.create(persistence);
    const importRecords = Array.from({ length: 33 }, (_, index) => ({
      firstName: `Imported${index + 1}`,
      fullName: `Imported${index + 1} Driver`,
      lastName: "Driver",
      raceNumber: `${index + 1}`,
      transponderNumber: `${index + 1}`,
    }));
    ...
    expect(
      getEntrantsForEvent(service.catalog, SEED_EVENT_ID).filter((entrant) =>
        entrant.name.startsWith("Imported"),
      ),
    ).toHaveLength(33);
  ...
  it("persists bulk session updates as one service notification", async () => {
    ...
    expect(
      service.catalog.sessions.find(
        (session) => session.id === SEED_RACE_SESSION_ID,
      )?.name,
    ).toBe("Bulk Race");
    expect(seededPersistence.save).toHaveBeenCalledTimes(1);
    expect(onPersistedLedger).toHaveBeenCalledTimes(1);
  ...
  it("syncs categories and entrants from imported event data IDs", async () => {
    ...
    expect(
      categories.find((category) => category.id === "cat-c"),
    ).toBeDefined();
    expect(event?.entrantIds).toEqual(
      expect.arrayContaining(["team-a", "team-b"]),
    );
    expect(teamA?.categoryIds).toEqual(
      expect.arrayContaining(["cat-a", "cat-b"]),
    );
    expect(teamA?.entrantType).toBe("team");
    expect(teamA?.teamMembers).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          firstName: "Pat",
          lastName: "Rider",
    ...
    expect(entrants.find((entrant) => entrant.id === "p1")).toEqual(
      expect.objectContaining({
        categoryId: "cat-a",
        entrantType: "rider",
        name: "Pat Rider",
    ...
    expect(entrants.find((entrant) => entrant.id === "p2")).toEqual(
      expect.objectContaining({
        categoryId: "cat-b",
        entrantType: "rider",
        name: "Quinn Rider",
  ...
  it("imports Apical race state as an event/session scaffold without activating it", async () => {
    ...
    const importedParticipantId = createEventParticipantId(
      "participant-apical-301",
    );
    ...
    const event = service.catalog.events.find(
      (item) => item.id === "7b83ad1e-54ba-5f00-9712-1c82d3178640",
    );
    const session = service.catalog.sessions.find(
      (item) => item.id === importedSessionId,
    );
    const entrant = service.catalog.entrants.find(
      (item) => item.id === importedEntrantId,
    );
    const importedCategory = getCategoriesForEvent(
      service.catalog,
      event!.id,
    ).find((category) => category.id === importedCategoryId);
    const timingErrorCategory = service.catalog.categories.find(
      (item) => item.name === "Timing Error List",
    );
    ...
    expect(
      Array.from(getCategoryAssignedSessionIds(importedCategory, [session!])),
    ).toEqual([importedSessionId]);
    expect(
      Array.from(
        getEntrantAssignedSessionIds(
          entrant,
          getCategoriesForEvent(service.catalog, event!.id),
          [session!],
          getEntrantsForEvent(service.catalog, event!.id),
        ),
      ),
    ).toEqual([importedSessionId]);
    expect(timingErrorCategory).toEqual(
      expect.objectContaining({
    ...
    expect(
      service.getImportedRaceStateMetadata(
    ...
    expect(seededPersistence.save).toHaveBeenLastCalledWith(
      expect.objectContaining({
        mutations: expect.arrayContaining([
          expect.objectContaining({
            apicalDataFilePath:
  ...
  it("imports an MR-SCATS catalog as event sessions, categories, entrants, and per-session race states", async () => {
    ...
    const qualifyingSessionId = createSessionId(
      "mr-scats-test-qualifying-session",
    );
    const raceCategoryId = createCategoryId("mr-scats-test-category-race");
    const qualifyingCategoryId = createCategoryId(
      "mr-scats-test-category-qualifying",
    );
    const raceEntrantId = createEventEntrantId("mr-scats-test-entrant-race");
    const qualifyingEntrantId = createEventEntrantId(
      "mr-scats-test-entrant-qualifying",
    );
    const raceParticipantId = createEventParticipantId(
      "mr-scats-test-participant-race",
    );
    const qualifyingParticipantId = createEventParticipantId(
      "mr-scats-test-participant-qualifying",
    );
    const raceRecordId = createTimeRecordId("mr-scats-test-race-record");
    const qualifyingRecordId = createTimeRecordId(
      "mr-scats-test-qualifying-record",
    );
    const raceSourceId = createTimeRecordSourceId("mr-scats-test-race-source");
    const qualifyingSourceId = createTimeRecordSourceId(
      "mr-scats-test-qualifying-source",
    );
      ...
      raceState: {
        categories: [
          ...
          },
          {
            code: "QUAL",
            description: "",
            id: qualifyingCategoryId,
            name: "Qualifying Class",
          },
        ],
        participants: [
          {
    ...
    const event = service.catalog.events.find(
      (item) => item.id === importedEventId,
    );
    const raceSession = service.catalog.sessions.find(
      (item) => item.id === raceSessionId,
    );
    const qualifyingSession = service.catalog.sessions.find(
      (item) => item.id === qualifyingSessionId,
    );
    const importedEntrants = getEntrantsForEvent(
      service.catalog,
      importedEventId,
    );
    const raceState = service.getImportedRaceState(
      importedEventId,
      raceSessionId,
    );
    const qualifyingRaceState = service.getImportedRaceState(
      importedEventId,
      qualifyingSessionId,
    );
    const latestPersistedLedger = vi
      .mocked(seededPersistence.save)
      .mock.calls.at(-1)?.[0];
    ...
    expect(qualifyingSession).toEqual(
      expect.objectContaining({
        categoryIds: [qualifyingCategoryId],
        kind: "qualifying",
        minimumLapTimeMilliseconds: 25_000,
    ...
    expect(importedEntrants).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          categoryIds: [raceCategoryId],
          id: raceEntrantId,
    ...
    expect(qualifyingRaceState?.participants).toEqual([
      expect.objectContaining({ id: qualifyingParticipantId }),
    ]);
    ...
    expect(raceState?.timeRecordSources).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          filePath: "W9721R01.DBF",
          id: raceSourceId,
    ...
    expect(seededPersistence.save).toHaveBeenCalledTimes(1);
    expect(onPersistedLedger).toHaveBeenCalledTimes(1);
    expect(latestPersistedLedger!.mutations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          event: expect.objectContaining({ id: importedEventId }),
          type: "event-created",
        }),
        expect.objectContaining({
          session: expect.objectContaining({ id: raceSessionId }),
          type: "session-created",
        }),
        expect.objectContaining({
          session: expect.objectContaining({ id: qualifyingSessionId }),
          type: "session-created",
        }),
        expect.objectContaining({
          eventId: importedEventId,
    ...
    expect(
      service
        .getImportedRaceState(importedEventId, raceSessionId)
        ?.records?.map((record) => record.id),
    ).toEqual([raceRecordId]);
    expect(
      service
        .getImportedRaceState(importedEventId, qualifyingSessionId)
        ?.records?.map((record) => record.id),
    ).toEqual([qualifyingRecordId]);
  });
  ...
  it("does not write duplicate scaffold mutations when the same event scaffold is synced twice", async () => {
    ...
    const saveCountAfterFirstSync = vi.mocked(seededPersistence.save).mock.calls
      .length;
    ...
    expect(vi.mocked(seededPersistence.save).mock.calls.length).toBe(
      saveCountAfterFirstSync,
    );
    expect(
      getCategoriesForEvent(service.catalog, SEED_EVENT_ID).map(
        (category) => category.id,
      ),
    ).toEqual(expect.arrayContaining([categoryAId, categoryBId]));
  });

  it("persists imported participant racePlate identifiers when scaffold entrants are created and updated", async () => {
    const seededPersistence = createPersistence(createSeedEventCatalogLedger());
    const service = await EventCatalogService.create(seededPersistence);
    const importedCategoryId = createCategoryId(
      "mr-scats-raceplate-scaffold-category",
    );
    const importedEntrantId = createEventEntrantId(
      "mr-scats-raceplate-scaffold-entrant",
    );
    const importedParticipantId = createEventParticipantId(
      "mr-scats-raceplate-scaffold-participant",
    );
    ...
    await service.syncEventScaffold(
      SEED_EVENT_ID,
      [importedCategory],
      [createParticipant("15")],
    );

    const createdEntrant = service.catalog.entrants.find(
      (entrant) => entrant.id === importedEntrantId,
    );
    const createdLedger = vi
      .mocked(seededPersistence.save)
      .mock.calls.at(-1)?.[0] as EventCatalogLedger;
    expect(createdEntrant).toEqual(
      expect.objectContaining({
    ...
    expect(createdLedger.mutations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          entrant: expect.objectContaining({
            id: importedEntrantId,
    ...
    await service.syncEventScaffold(
      SEED_EVENT_ID,
      [importedCategory],
      [createParticipant("16")],
    );

    const updatedEntrant = service.catalog.entrants.find(
      (entrant) => entrant.id === importedEntrantId,
    );
    const updatedLedger = vi
      .mocked(seededPersistence.save)
      .mock.calls.at(-1)?.[0] as EventCatalogLedger;
    expect(updatedEntrant).toEqual(
      expect.objectContaining({
    ...
    expect(updatedEntrant?.identifiers).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ racePlate: "15" })]),
    );
    expect(updatedLedger.mutations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it("treats a duplicate Apical import as a no-op once the event state already matches", async () => {
    ...
    const saveCountAfterFirstImport = vi.mocked(seededPersistence.save).mock
      .calls.length;

    await service.importApicalRaceState(importData);

    expect(vi.mocked(seededPersistence.save).mock.calls.length).toBe(
      saveCountAfterFirstImport,
    );
    expect(
      service.getImportedRaceStateMetadata(importedEventId, importedSessionId),
    ).toEqual(
      expect.objectContaining({
        raceState: expect.objectContaining({
          categories: expect.arrayContaining([
          ...
          participants: expect.arrayContaining([
            expect.objectContaining({ id: importedParticipantId }),
          ]),
        }),
      }),
    );
  ...
  it("matches re-imported categories and participants by stable values when upstream IDs change", async () => {
    const seededPersistence = createPersistence(
      createDefaultEventCatalogLedger(),
    );
    const service = await EventCatalogService.create(seededPersistence);
    const importedEventId = createEventId("apical-stable-value-event");
    const importedSessionId = createSessionId("apical-stable-value-session");
    const firstCategoryId = createCategoryId(
      "apical-stable-value-category-first",
    );
    const secondCategoryId = createCategoryId(
      "apical-stable-value-category-second",
    );
    const firstEntrantId = createEventEntrantId(
      "apical-stable-value-entrant-first",
    );
    const secondEntrantId = createEventEntrantId(
      "apical-stable-value-entrant-second",
    );
    const firstParticipantId = createEventParticipantId(
      "apical-stable-value-participant-first",
    );
    const secondParticipantId = createEventParticipantId(
      "apical-stable-value-participant-second",
    );
    const createImportData = (
      categoryId: EventCategoryId,
      entrantId: EventEntrantId,
      participantId: EventParticipantId,
    ): Parameters<typeof service.importApicalRaceState>[0] => ({
      eventDate: "2026-06-07T01:30:00.000Z",
      eventId: importedEventId,
      eventName: "Apical Stable Values Round",
      raceState: {
        categories: [
          { code: "PRO-A", description: "", id: categoryId, name: "Pro A" },
        ],
        participants: [
          {
            categoryId,
    ...
    await service.importApicalRaceState(
      createImportData(firstCategoryId, firstEntrantId, firstParticipantId),
    );
    const saveCountAfterFirstImport = vi.mocked(seededPersistence.save).mock
      .calls.length;

    await service.importApicalRaceState(
      createImportData(secondCategoryId, secondEntrantId, secondParticipantId),
    );
    ...
    expect(
      getCategoriesForEvent(service.catalog, importedEventId),
    ).toHaveLength(1);
    expect(getEntrantsForEvent(service.catalog, importedEventId)).toEqual([
      expect.objectContaining({
        id: firstEntrantId,
        memberParticipantIds: [firstParticipantId],
      }),
    ]);
    expect(
      service.getImportedRaceStateMetadata(importedEventId, importedSessionId)
  ...
  it("compacts superseded imported race state mutations for the same event session", async () => {
    ...
    const createImportData = (
      ...
      raceState: {
        categories: [
          { code: "A", description: "", id: importedCategoryId, name: "A" },
        ],
        participants: [
          {
            categoryId: importedCategoryId,
    ...
    await service.importApicalRaceState(
      createImportData(
        "compacted-import-record-old",
        "2026-06-07T01:45:00.000Z",
      ),
    );
    await service.importApicalRaceState(
      createImportData(
        "compacted-import-record-new",
        "2026-06-07T01:46:00.000Z",
      ),
    );
    ...
    const savedLedger = vi
      .mocked(seededPersistence.save)
      .mock.calls.at(-1)?.[0] as EventCatalogLedger;
    const importMutations = savedLedger.mutations.filter(
      (
        mutation,
      ): mutation is Extract<
        EventCatalogLedger["mutations"][number],
        { type: "race-state-imported" }
      > =>
    ...
    expect(importMutations[0]?.raceState.records).toEqual([
      expect.objectContaining({
        id: createTimeRecordId("compacted-import-record-new"),
      }),
    ]);
  });
  ...
  it("scaffolds imported Apical teams using the imported team name and category", async () => {
    ...
    const teamEntrant = service.catalog.entrants.find(
      (entrant) => entrant.id === importedTeamId,
    );
    const memberEntrants = service.catalog.entrants.filter(
      (entrant) => entrant.teamEntrantId === importedTeamId,
    );
    const importedCategory = service.catalog.categories.find(
      (category) => category.id === importedCategoryId,
    );

    expect(teamEntrant).toEqual(
      expect.objectContaining({
        categoryId: importedCategoryId,
        categoryIds: [importedCategoryId],
        entrantType: "team",
    ...
    expect(
      Array.from(
        getCategoryAssignedSessionIds(
          importedCategory,
          getSessionsForEvent(service.catalog, importedEventId),
        ),
      ),
    ).toEqual([importedSessionId]);
    expect(
      Array.from(
        getEntrantAssignedSessionIds(
          ...
          getCategoriesForEvent(service.catalog, importedEventId),
          getSessionsForEvent(service.catalog, importedEventId),
          getEntrantsForEvent(service.catalog, importedEventId),
        ),
      ),
    ).toEqual([importedSessionId]);
    expect(memberEntrants).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          id: riderOneId,
          name: "Alice RIDER",
  ...
  it("imports real Apical Results team rows into catalog state, persisted ledger, reloads, and team selectors", async () => {
    ...
    const importedSessionId = createSessionId(
      "apical-event-68-real-ledger-session",
    );
    const apicalData = await readApicalExcelBuffer(
      createApicalTeamNameDisplayWorkbookBuffer(),
    );
    ...
    const expectedRaceNumbers =
      teamResult?.RaceNumbers.split(",").map((raceNumber) =>
        raceNumber.trim(),
      ) || [];
    ...
    const raceStateTeam = raceState.teams?.find(
      (team) => team.name === "The Evereadys",
    );
    const expectedMemberParticipants = expectedRaceNumbers.map((raceNumber) => {
      return raceState.participants?.find(
        (participant) => getParticipantRacePlate(participant) === raceNumber,
      );
    });
    ...
    expect([...raceStateTeam!.members].sort()).toEqual(
      expectedMemberParticipants.map((participant) => participant!.id).sort(),
    );
    ...
    await service.importApicalRaceState({
      apicalDataFilePath: path.join(
        process.cwd(),
        "src",
        "generated",
        "apical-excel-cache",
        "apical-event-68.xlsx",
      ),
    ...
    const category = getCategoriesForEvent(
      service.catalog,
      importedEventId,
    ).find((item) => item.name === "EBIKE_TEAM");
    const teamEntrant = eventEntrants.find(
      (entrant) => entrant.name === "The Evereadys",
    );
    ...
    const importedEvent = service.catalog.events.find(
      (event) => event.id === importedEventId,
    );
    const latestPersistedLedger = vi
      .mocked(seededPersistence.save)
      .mock.calls.at(-1)?.[0];
    const latestPushedLedger = vi
      .mocked(onPersistedLedger)
      .mock.calls.at(-1)?.[0];
    ...
    expect(importedEvent?.entrantIds).toEqual(
      expect.arrayContaining([
        teamEntrant!.id,
    ...
    expect(
      getTeamsForParticipant(
        service.catalog,
        importedEventId,
        expectedMemberParticipants[0]!.id,
      ),
    ).toEqual([teamEntrant]);

    expect(latestPersistedLedger).toBeDefined();
    expect(latestPushedLedger).toBeDefined();
    expect(latestPushedLedger?.mutations).toHaveLength(
      latestPersistedLedger!.mutations.length,
    );
    expect(latestPersistedLedger!.mutations).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          changes: expect.objectContaining({
            entrantIds: expect.arrayContaining([teamEntrant!.id]),
          }),
          type: "event-updated",
        }),
      ]),
    ...
    const reloadedService = await EventCatalogService.create(
      createPersistence(latestPersistedLedger),
    );
    const reloadedTeamEntrant = getEntrantsForEvent(
      reloadedService.catalog,
      importedEventId,
    ).find((entrant) => entrant.id === teamEntrant!.id);
    expect(reloadedTeamEntrant).toEqual(teamEntrant);
    expect(
      getTeamsForParticipant(
        reloadedService.catalog,
        importedEventId,
        expectedMemberParticipants[1]!.id,
      ),
    ).toEqual([teamEntrant]);

    const addedParticipantId = createEventParticipantId(
      "apical-event-68-real-ledger-added-member",
    );
    const updatedLedger: EventCatalogLedger = {
      ...
      mutations: [
        ...latestPersistedLedger!.mutations,
        {
          changes: {
            memberParticipantIds: [
              ...teamEntrant!.memberParticipantIds,
              addedParticipantId,
            ],
            teamMembers: [
  ...
  it("overrides Apical import scaffold data on re-fetch while preserving earlier mutations for matching IDs", async () => {
    ...
    await service.updateCategory(importedCategoryId, {
      name: "Manual Category Edit",
    });
    await service.updateEntrant(importedEntrantId, {
      firstName: "Manual",
      name: "Manual Rider Edit",
    });
    ...
    const category = service.catalog.categories.find(
      (item) => item.id === importedCategoryId,
    );
    const entrant = service.catalog.entrants.find(
      (item) => item.id === importedEntrantId,
    );
  ...
  it("links imported categories to their imported sessions while reusing event categories", async () => {
    ...
    const sharedCategoryId = createCategoryId(
      "multi-session-import-category-shared",
    );
    const firstOnlyCategoryId = createCategoryId(
      "multi-session-import-category-first-only",
    );
    const secondOnlyCategoryId = createCategoryId(
      "multi-session-import-category-second-only",
    );
    ...
    const eventCategories = getCategoriesForEvent(
      service.catalog,
      importedEventId,
    );
    const eventSessions = getSessionsForEvent(service.catalog, importedEventId);
    const sharedCategory = eventCategories.find(
      (category) => category.id === sharedCategoryId,
    );
    const firstOnlyCategory = eventCategories.find(
      (category) => category.id === firstOnlyCategoryId,
    );
    const secondOnlyCategory = eventCategories.find(
      (category) => category.id === secondOnlyCategoryId,
    );
    ...
    expect(
      eventCategories.filter((category) => category.id === sharedCategoryId),
    ).toHaveLength(1);
    expect(
      Array.from(getCategoryAssignedSessionIds(sharedCategory, eventSessions)),
    ).toEqual([firstSessionId, secondSessionId]);
    expect(
      Array.from(
        getCategoryAssignedSessionIds(firstOnlyCategory, eventSessions),
      ),
    ).toEqual([firstSessionId]);
    expect(
      Array.from(
        getCategoryAssignedSessionIds(secondOnlyCategory, eventSessions),
      ),
    ...
    ).toEqual([sharedCategoryId, firstOnlyCategoryId]);
    expect(
      Array.from(
        getSessionAssignedCategoryIds(
          eventCategories,
    ...
    ).toEqual([sharedCategoryId, secondOnlyCategoryId]);
  });
  ...
  it("reloads imported race state while replaying manual category and entrant mutations above the refreshed scaffold", async () => {
    ...
    const savedLedger = vi
      .mocked(seededPersistence.save)
      .mock.calls.at(-1)?.[0] as EventCatalogLedger;
    const finalManualCategoryUpdateIndex = savedLedger.mutations.findLastIndex(
      (mutation) =>
        mutation.type === "category-updated" &&
        mutation.changes.name === "Manual Category Name",
    );
    const finalManualEntrantUpdateIndex = savedLedger.mutations.findLastIndex(
      (mutation) =>
        mutation.type === "entrant-updated" &&
        mutation.changes.name === "Manual Rider Name",
    );
    const refreshedImportIndex = savedLedger.mutations.findLastIndex(
      (mutation) => mutation.type === "race-state-imported",
    );
    expect(seededPersistence.save).toHaveBeenCalledTimes(1);
    expect(onPersistedLedger).toHaveBeenCalledTimes(1);
    ...
    expect(importedRaceState).toEqual(
      expect.objectContaining({
        categories: [
          expect.objectContaining({
            id: importedCategoryId,
    ...
    expect(finalManualCategoryUpdateIndex).toBeGreaterThan(
      refreshedImportIndex,
    );
  ...
  it("does not replay stale scaffold deletions for entrants and categories that still exist in refreshed import data", async () => {
    ...
    const importedEntrantId = createEventEntrantId(
      "reload-stale-delete-entrant",
    );
    const importedParticipantId = createEventParticipantId(
      "reload-stale-delete-participant",
    );
        ...
        categories: [
          {
            code: "A",
            description: "",
            id: "reload-stale-delete-category",
            name: "A",
          },
        ],
        participants: [
    ...
    const reloadedService =
      await EventCatalogService.create(reloadedPersistence);

    expect(
      getEntrantsForEvent(reloadedService.catalog, importedEventId).find(
        (entrant) => entrant.id === importedEntrantId,
      ),
    ).toBeUndefined();

    await reloadedService.reloadImportedRaceState(
      importedEventId,
      importedSessionId,
      {
        categories: [
    ...
    const savedLedger = vi
      .mocked(reloadedPersistence.save)
      .mock.calls.at(-1)?.[0] as EventCatalogLedger;
    const refreshedImportIndex = savedLedger.mutations.findLastIndex(
      (mutation) => mutation.type === "race-state-imported",
    );
    const replayedDeletion = savedLedger.mutations
      .slice(refreshedImportIndex + 1)
      .find((mutation) => {
        return (
          (mutation.type === "entrant-deleted" &&
            mutation.entrantId === importedEntrantId) ||
          (mutation.type === "category-deleted" &&
            mutation.categoryId === importedCategoryId)
        );
      });
    const entrant = getEntrantsForEvent(
      reloadedService.catalog,
      importedEventId,
    ).find((item) => item.id === importedEntrantId);
    const session = getSessionsForEvent(
      reloadedService.catalog,
      importedEventId,
    ).find((item) => item.id === importedSessionId);
    const importedRaceState = reloadedService.getImportedRaceState(
      importedEventId,
      importedSessionId,
    );
  ...
  it("deduplicates duplicate imported participants and purges duplicate entrants during reload", async () => {
    ...
    const importedEntrantId = createEventEntrantId(
      "reload-deduplicate-entrant",
    );
    const importedParticipantId = createEventParticipantId(
      "reload-deduplicate-participant",
    );
    const duplicateEntrantId = createEventEntrantId(
      "reload-deduplicate-entrant-duplicate",
    );
    const seededPersistence = createPersistence(createSeedEventCatalogLedger());
    const service = await EventCatalogService.create(seededPersistence);
        ...
        records: [
          {
            ...
            recordType: RECORD_TX_CROSSING,
            sequence: 1,
            source: createTimeRecordSourceId(
              "reload-deduplicate-source-initial",
            ),
            time: new Date("2026-06-07T01:45:00.000Z"),
          } as EventTimeRecord,
        ],
    ...
    const reloadedService =
      await EventCatalogService.create(reloadedPersistence);
    const duplicateIncomingParticipantIdA = createEventParticipantId(
      "reload-deduplicate-incoming-a",
    );
    const duplicateIncomingParticipantIdB = createEventParticipantId(
      "reload-deduplicate-incoming-b",
    );
    const duplicateIncomingEntrantIdA = createEventEntrantId(
      "reload-deduplicate-incoming-entrant-a",
    );
    const duplicateIncomingEntrantIdB = createEventEntrantId(
      "reload-deduplicate-incoming-entrant-b",
    );
    ...
    const savedLedger = vi
      .mocked(reloadedPersistence.save)
      .mock.calls.at(-1)?.[0] as EventCatalogLedger;
    const eventEntrants = getEntrantsForEvent(
      reloadedService.catalog,
      importedEventId,
    );
    const importedRaceState = reloadedService.getImportedRaceState(
    ...
    const canonicalEntrant = eventEntrants.find(
      (entrant) => entrant.id === importedEntrantId,
    );

    expect(canonicalEntrant).toEqual(
      expect.objectContaining({
        categoryId: importedCategoryId,
        id: importedEntrantId,
        memberParticipantIds: [importedParticipantId],
    ...
    expect(
      eventEntrants.find((entrant) => entrant.id === duplicateEntrantId),
    ).toBeUndefined();
    expect(
      eventEntrants.filter((entrant) => entrant.name === "Casey Driver"),
    ).toHaveLength(1);
  ...
  it("normalizes primary entrant category updates and clears deleted category references", async () => {
    ...
    let entrant = service.catalog.entrants.find(
      (item) => item.id === SEED_ENTRANT_ID,
    );
    expect(entrant).toEqual(
      expect.objectContaining({
    ...
    entrant = service.catalog.entrants.find(
      (item) => item.id === SEED_ENTRANT_ID,
    );
  ...
  it("migrates entrants from named placeholder categories when a later import identifies the category", async () => {
    const seededPersistence = createPersistence(createSeedEventCatalogLedger());
    const service = await EventCatalogService.create(seededPersistence);
    const placeholderCategoryId = createCategoryId(
      "multi-stage-placeholder-premier",
    );
    const identifiedCategoryId = createCategoryId(
      "multi-stage-identified-premier",
    );
    const teamEntrantId = createEventEntrantId("multi-stage-premier-team");
    const firstParticipantId = createEventParticipantId(
      "multi-stage-premier-first",
    );
    const secondParticipantId = createEventParticipantId(
      "multi-stage-premier-second",
    );
    ...
    expect(
      service.catalog.categories.find(
        (category) => category.id === placeholderCategoryId,
      )?.isPlaceholder,
    ).toBe(true);
    expect(
      service.catalog.entrants.find((entrant) => entrant.id === teamEntrantId),
    ).toEqual(
      expect.objectContaining({
        categoryId: placeholderCategoryId,
        categoryIds: [placeholderCategoryId],
      }),
    );
    ...
    const migratedTeam = service.catalog.entrants.find(
      (entrant) => entrant.id === teamEntrantId,
    );
    expect(migratedTeam).toEqual(
      expect.objectContaining({
        categoryId: identifiedCategoryId,
        categoryIds: [identifiedCategoryId],
      }),
    );
    expect(
      migratedTeam?.teamMembers?.map((member) => member.categoryId),
    ).toEqual([identifiedCategoryId, identifiedCategoryId]);
  });

  it("uses the selected import category to replace matched placeholder entrant and participant categories", async () => {
    const seededPersistence = createPersistence(createSeedEventCatalogLedger());
    const service = await EventCatalogService.create(seededPersistence);
    const seedEntrant = service.catalog.entrants.find(
      (entrant) => entrant.id === SEED_ENTRANT_ID,
    )!;
    const participantId = seedEntrant.memberParticipantIds[0]!;
    const unmatchedParticipantId = createEventParticipantId(
      "placeholder-participant-without-catalog-entrant",
    );
    const unmatchedEntrantId = createEventEntrantId(
      "placeholder-participant-without-catalog-entrant",
    );
    ...
    await service.updateEntrant(SEED_ENTRANT_ID, {
      categoryId: SEED_CLUBMAN_CATEGORY_ID,
      identifiers: [
        { fromTime: undefined, toTime: undefined, txNo: "91" },
      ] as ParticipantTransponder[],
    });
    await service.updateImportedRaceState(SEED_EVENT_ID, SEED_RACE_SESSION_ID, {
      categories: getCategoriesForEvent(service.catalog, SEED_EVENT_ID),
      participants: [
        ...
        },
        {
          categoryId: SEED_CLUBMAN_CATEGORY_ID,
          currentResult: undefined,
          entrantId: unmatchedEntrantId,
          firstname: "Unscaffolded",
          id: unmatchedParticipantId,
          identifiers: [
            { fromTime: undefined, toTime: undefined, txNo: "92" },
          ] as ParticipantTransponder[],
          lastRecordTime: null,
          resultDuration: null,
          surname: "Driver",
    ...
    await service.importEntrants(
      ...
      [
        ...
        },
        {
          firstName: "Unscaffolded",
          lastName: "Driver",
          transponderNumber: "92",
        },
      ],
      SEED_PREMIER_CATEGORY_ID,
    );

    const importedEntrant = service.catalog.entrants.find(
      (entrant) => entrant.id === SEED_ENTRANT_ID,
    );
    const importedParticipant = service
      .getImportedRaceState(SEED_EVENT_ID, SEED_RACE_SESSION_ID)
      ?.participants?.find((participant) => participant.id === participantId);
    const importedUnscaffoldedParticipant = service
      .getImportedRaceState(SEED_EVENT_ID, SEED_RACE_SESSION_ID)
      ?.participants?.find(
        (participant) => participant.id === unmatchedParticipantId,
      );
    expect(importedEntrant).toEqual(
      expect.objectContaining({
        categoryId: SEED_PREMIER_CATEGORY_ID,
        categoryIds: [SEED_PREMIER_CATEGORY_ID],
      }),
    ...
    expect(importedUnscaffoldedParticipant?.entrantId).toBe(unmatchedEntrantId);
  });

  it("uses the selected category and assigns both race plate and transmitter from entrant files", async () => {
    const seededPersistence = createPersistence(createSeedEventCatalogLedger());
    const service = await EventCatalogService.create(seededPersistence);
    await service.updateImportedRaceState(SEED_EVENT_ID, SEED_RACE_SESSION_ID, {
      categories: getCategoriesForEvent(service.catalog, SEED_EVENT_ID),
    ...
    await service.importEntrants(
      ...
      [
        {
          firstName: "Ashley",
          fullName: "Ashley Goldstraw",
          lastName: "Goldstraw",
          raceNumber: "15",
          transponderNumber: "200015",
        },
      ],
      SEED_PREMIER_CATEGORY_ID,
    );
    ...
    const entrant = getEntrantsForEvent(service.catalog, SEED_EVENT_ID).find(
      (candidate) => candidate.name === "Ashley Goldstraw",
    );
    const participant = service
      .getImportedRaceState(SEED_EVENT_ID, SEED_RACE_SESSION_ID)
      ?.participants?.find((candidate) => candidate.entrantId === entrant?.id);
  ...
  it("falls back to participant ID when imported entrant ID is blank", async () => {
    ...
    const entrant = service.catalog.entrants.find(
      (item) => item.id === "participant-fallback-id",
    );
    expect(entrant).toEqual(
      expect.objectContaining({
        firstName: "Blank",
  ...
  it("creates and deletes session/category/entrant through immutable mutations", async () => {
    ...
    const createdSessionId = sessions.find(
    ...
    const createdCategoryId = categories.find(
    ...
    const createdEntrantId = entrants.find(
      (entrant) => entrant.name === "New Driver",
    )?.id;
    expect(createdEntrantId).toBeDefined();
    await service.deleteEntrant(
      SEED_EVENT_ID,
      createdEntrantId!,
      onCompleteStep,
    );
    ...
    expect(
      categories.find((category) => category.id === createdCategoryId),
    ).toBeUndefined();
    expect(
      entrants.find((entrant) => entrant.id === createdEntrantId),
    ).toBeUndefined();
  });
});
...
describe("applyEventCatalogLedger", () => {
  it("applies one mutation to an already rebuilt state", () => {
    ...
    const updatedState = applyEventCatalogMutation(state, {
      changes: { name: "Incrementally Updated Event" },
      eventId: SEED_EVENT_ID,
      id: createId("incremental-event-update"),
      timestamp: "2026-07-18T00:00:00.000Z",
      type: "event-updated",
    });

    expect(
      updatedState.events.find((event) => event.id === SEED_EVENT_ID)?.name,
    ).toBe("Incrementally Updated Event");
    expect(
      state.events.find((event) => event.id === SEED_EVENT_ID)?.name,
    ).not.toBe("Incrementally Updated Event");
  });
  ...
  it("keeps deleted historical category versions while exposing only one active category for an ID", () => {
    ...
    const categoryVersions = state.categories.filter(
      (category) => category.id === SEED_CLUBMAN_CATEGORY_ID,
    );
    const activeCategoryVersions = categoryVersions.filter(
    ...
    expect(
      categoryVersions.some(
        (category) =>
          category.deleted === true && category.name !== "Clubman Recreated",
      ),
    ).toBe(true);
    ...
    expect(
      getCategoriesForEvent(state, SEED_EVENT_ID).filter(
        (category) => category.id === SEED_CLUBMAN_CATEGORY_ID,
      ),
    ).toEqual(activeCategoryVersions);
  });
});
