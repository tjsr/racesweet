...
describe('calculateYellowFlagPeriods', () => {
  it('pairs yellow flags with a green removal and reports leader laps and duration', () => {
    const periods = calculateYellowFlagPeriods(
      [
        { flagType: 'yellow', flagValue: 'caution', id: createTimeRecordId('yellow'), recordType: EVENT_FLAG_DISPLAYED, sequence: 1, source, time: at(10) },
        { flagType: 'green', flagValue: 'course', id: createTimeRecordId('green'), indicatesRaceStart: false, recordType: EVENT_FLAG_DISPLAYED, sequence: 2, source, time: at(25) },
      ],
  ...
  it('recognises a retracted yellow record as the removal and filters by category', () => {
    const periods = calculateYellowFlagPeriods(
      [
        { categoryIds: ['other-category'], flagType: 'yellow', flagValue: 'caution', id: createTimeRecordId('other-yellow'), recordType: EVENT_FLAG_DISPLAYED, sequence: 1, source, time: at(1) },
        { categoryIds: ['category-a'], flagType: 'yellow', flagValue: 'caution', id: createTimeRecordId('yellow-a'), recordType: EVENT_FLAG_DISPLAYED, sequence: 2, source, time: at(2) },
        { categoryIds: ['category-a'], flagType: 'yellow', flagValue: 'caution', id: createTimeRecordId('yellow-a-removal'), recordType: EVENT_FLAG_DISPLAYED | 8, sequence: 3, source, time: at(5) },
      ],
      [],
      'category-a',
    );
